#!/usr/bin/env bash
# Noise sweep on Hindmarsh–Rose — counterpart to run_lowdata_hr.sh.
# Mirror of run_noise_rossler.sh but with HR-specific seq_len=200.
# Validation trajectory is ALWAYS clean (handled inside train.py).
#
# WAITS by default until the HR low-data PIDs in
# /tmp/lowdata_hr_pids.txt are done; override with WAIT=0.
...
if [[ "$WAIT" == "1" && -f /tmp/lowdata_hr_pids.txt ]]; then
  echo "=== Waiting for HR low-data sweep PIDs to finish (WAIT=0 to skip) ==="
  while read -r pid; do
    [[ -z "$pid" ]] && continue
    while kill -0 "$pid" 2>/dev/null; do sleep 60; done
    echo "  PID $pid done at $(date)"
  done < /tmp/lowdata_hr_pids.txt
  if [[ -f /tmp/lowdata_hr_fv_pid.txt ]]; then
    fv=$(cat /tmp/lowdata_hr_fv_pid.txt)
    while kill -0 "$fv" 2>/dev/null; do sleep 30; done
    echo "  HR low-data FV $fv done at $(date)"
  fi
  echo "=== HR low-data sweep finished, proceeding at $(date) ==="
fi
...
printf '%s\n' "${BG_PIDS[@]}" > /tmp/noise_hr_pids.txt
printf '%s\n' "${TAGS[@]}"    > /tmp/noise_hr_tags.txt
...
) > logs/_noise_hr_finalvalidate.out 2>&1 &
FV=$!
echo "Final-validator PID=$FV"
echo "$FV" > /tmp/noise_hr_fv_pid.txt
