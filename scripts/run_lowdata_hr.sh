#!/usr/bin/env bash
# Low-data sweep on Hindmarsh–Rose.
# Mirror of run_lowdata_rossler.sh — same 4 models × 5 fractions.
# Settings match the HR P1D-full main run:
#   seq_len=200, hidden=64, aug=4, curriculum "20:1,50:1,85:1,150:2",
#   tvalid_threshold=1.0.  csode_P1D_full row adds the full recipe.
#
# Tag convention:
#   hr_{recipe}_lowdata_d{XXX}    where XXX = 100*frac as %03d.
# Checkpoint filename inside checkpoints/<tag>/  is  hindmarsh_rose_<model>.pt
# (where model = "csode" for csode_P1D_full).
...
printf '%s\n' "${BG_PIDS[@]}" > /tmp/lowdata_hr_pids.txt
printf '%s\n' "${TAGS[@]}"    > /tmp/lowdata_hr_tags.txt
...
) > logs/_lowdata_hr_finalvalidate.out 2>&1 &
FV=$!
echo "Final-validator PID=$FV"
echo "$FV" > /tmp/lowdata_hr_fv_pid.txt
